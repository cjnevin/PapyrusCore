...
extension Papyrus {
    ...
    func possibilities(player: Player, lexicon: Lexicon) -> [BoundaryTiles]? {
        guard let source = lexicon.dictionary else { return nil }
        ...
        for boundary in boundaries {
            ...
            lexicon.anagramsOf(letters, length: boundary.length + 1, prefix: "",
                fixedLetters: indexedCharacters, fixedCount: indexedCharacters.count,
                root: source, results: &results)
