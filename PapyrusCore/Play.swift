...
public typealias PlayAISquareTiles = [(Square, Tile)]
public typealias PlayAIOutput = (score: Int, squareTiles: PlayAISquareTiles)
...
extension Papyrus {
    ...
    public func playAI(lexicon: Lexicon) throws -> PlayAIOutput {
        ...
        guard let possibles = possibilities(player, lexicon: lexicon) else { throw ValidationError.NoOptions }
        ...
        for boundaryTiles in possibles {
            ...
            do {
                let points = try play(aiBoundary, submit: false, lexicon: lexicon)
